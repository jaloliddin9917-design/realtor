...
export function AddUserDialog() {
  ...
  useEffect(() => { if (added) { setOpen(false); setName(""); setPhone(""); setPassword(""); setRole("agent"); } }, [added]);
  return (
    <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) close(); }}>
      ...
      <DialogContent>
        ...
        <form className="flex flex-col gap-3" onSubmit={(e) => { e.preventDefault(); request({ name: name.trim(), phone: phone.trim(), password, role }); }}>
          ...
          <div className="flex flex-col gap-1.5"><Label htmlFor="user-password">{t("auth.password")}</Label><Input id="user-password" type="password" autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8} /></div>
