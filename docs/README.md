...
Status: docs are **Draft v0.1 (2026-08-29)**. Code: plans M0-1 (backend core — schema, parsing, persistence, photos, dedupe, properties, pipeline) and M0-2 (Telegram and OLX adapters, manual ingestion by link or form, the worker loop, the operator CLI) are implemented, with a real-PostgreSQL test suite; the implementation plans live in `superpowers/plans/`. Next is M0-3 (the API and the web panel). Mockups are Draft v0.1 too; UI copy is Uzbek (Latin) with fictional sample data.
